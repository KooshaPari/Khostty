...
| 9.15 | Write install docs + verification steps | 10m | 9.11-9.14 | Test each installer | DONE (`cd91fb549`, `docs/INSTALL.md`; status table corrected 2026-09-18 and again **2026-09-19** → **6 of 7 artifacts built, 1 NOT BUILT (GTK app)**; runtime halves explicitly open for the macOS `.app` (no GUI launch) and the Windows binaries (no GUI window)) |
...
| Install docs | `docs/INSTALL.md` | status table **6 of 7 artifacts built, 1 NOT BUILT (GTK app)**, as of 2026-09-19 |
...
- All platform installers run and launch — **PARTIALLY MET**: the Windows installation `.exe` is now built, installs, runs and uninstalls cleanly (2026-09-19); the GTK application `.deb` still does not exist (cross-compile blocker), and the macOS `.app` was still never launched in a GUI session. See `docs/INSTALL.md` and `docs/changelog/0.1.0.md` §4.
...
| G9 Docs+Packaging | 15 | 150 | ✅ 15/15 rows present; 9.11 build+sign+binary-exec verified, **GUI launch NOT performed**; 9.13 Windows installer **compiled + install/uninstall-verified 2026-09-19**; GTK .deb not buildable on this host |
| G10 Release | 9 | 90 | 🔶 IN PROGRESS — 4/9 DONE (10.1, 10.3, 10.4, 10.7) + 10.2 **partially** DONE (Windows installer built + install-verified; GTK app `.deb` still missing); 10.5/10.6/10.8 blocked on publish authorization |

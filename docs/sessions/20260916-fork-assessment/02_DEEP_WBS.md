...
| **Windows artifacts** | `file` + `objdump -p` on `zig-out/bin/ghostty.exe` / `ghostty-vt.dll` | exe 43,470,336 B `PE32+ executable (GUI) x86-64`; dll 7,545,344 B `PE32+ executable (DLL)`; **198 distinct `ghostty_*` functions in the DLL export table**; sha256 `df0b4c87…` / `b4cff87e…`. NOT executed: no Windows host, `wine` is absent, and every Homebrew wine cask is disabled (Gatekeeper, since 2026-09-01) with Rosetta not installed, so x86_64 Windows emulation is unreachable here. |

...
## G9: Docs + Packaging (DONE — 15/15 tasks; macOS GUI launch and GTK GUI-launch half of the Linux .deb both resolved (macOS `.app` non-interactive liveness only; GTK app headless Xvfb APP_ALIVE)) — MEDIUM PRIORITY
...
| 9.15 | Write install docs + verification steps | 10m | 9.11-9.14 | Test each installer | DONE (`cd91fb549`, `docs/INSTALL.md`; status table corrected 2026-09-18, **2026-09-19** and again **2026-09-19 (later)** → **7 of 7 artifacts BUILT; install-verify halves explicitly open for the macOS `.app` (no GUI launch) and the Windows binaries (no GUI window); the GTK app `.deb` is BUILT + INSTALL-VERIFIED + GUI-LAUNCH-VERIFIED (headless Xvfb) on the WSL glibc 2.43 host) |
...
- All platform installers run and launch — **PARTIALLY MET**: the Windows installation `.exe` is now built, installs, runs and uninstalls cleanly (2026-09-19); the GTK application `.deb` **is now built AND install-verified** (2026-09-19, WSL Fedora 44 host, glibc 2.43; negative control refused on Debian 12; see 9.12) but has not been launched as a windowed GTK app (no desktop session there); and the macOS `.app` was still never launched in a GUI session. See `docs/INSTALL.md` and `docs/changelog/0.1.0.md` §4.

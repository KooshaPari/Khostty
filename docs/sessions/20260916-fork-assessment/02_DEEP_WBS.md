...
| G9 | Documentation + Packaging | 15 | 150m | DONE (15/15; macOS .app builds+signs+verifies and its binary executes (GUI launch NOT performed; see 9.11); **GTK .deb BUILT 2026-09-19 in WSL** (see 9.12); Linux library .deb installs+runs on Debian 12; Windows exe executed) | MEDIUM |
...
| 9.12 | Package Linux .deb (and optionally .rpm) | 10m | G1 | dpkg packaging, desktop entry | **DONE 2026-09-19 — the GTK application .deb BUILT in WSL** (commits `9daf736e8`, `45e6d086b`). Two blockers fixed: (1) `deb.sh` passed `-Dtarget=x86_64-linux-gnu` unconditionally, which on a native x86_64 Linux host makes Zig treat the build as a cross-compile and refuse to link system shared libraries (`unable to find dynamic system library 'gobject-2.0' ... searched paths: none`); the target is now conditional on the host arch, and the native build succeeds. (2) The icon block wrote numbered ImageMagick files (`-0`..`-4`) into one directory and the desktop entry hard-coded `Icon=com.mitchellh.ghostty`; it now installs size-matched hicolor PNGs (16/24/32/48/256/512) and uses `Icon=@APPID@`. Artifact: `dist/khostty_0.1.0_amd64.deb`, 18,143,936 B, `sha256 209ba5ed…713a`, built in WSL Fedora 44 on `kooshapari-desk` (host has gtk4-devel, libadwaita-devel, gtk4-layer-shell-devel). Evidence `evidence/gtk_deb_build_2026-09-19.txt`. The library payload `.deb` (`khostty-vt_0.1.0_amd64.deb`) remains install-and-run-verified on x86-64 Debian 12 (evidence rows below). |
...
| 9.15 | Write install docs + verification steps | 10m | 9.11-9.14 | Test each installer | DONE (`cd91fb549`, `docs/INSTALL.md`; status table corrected 2026-09-18, **2026-09-19** and again **2026-09-19 (later)** → **7 of 7 artifacts BUILT; install-verify halves explicitly open for the macOS `.app` (no GUI launch), the GTK app `.deb` (no install/GUI launch) and the Windows binaries (no GUI window)) |

**Acceptance criteria**:
- All docs committed, cross-linked, no dead links
- macOS .app installs and runs (verified outside source tree)
- Linux .deb installs and runs — **SATISFIED for the libghostty-vt library payload** (`khostty-vt_0.1.0_amd64.deb`: `dpkg -i` exit 0, installed library linked and run, exit 0, in x86-64 Debian 12). The **GTK application payload is now BUILT** (2026-09-19, WSL Fedora 44): `khostty_0.1.0_amd64.deb`, 18,143,936 B, `sha256 209ba5ed…713a`, with hicolor icon set and `Icon=com.khostty.Khostty`. **Install-verify for the GTK payload is still OPEN** (no `dpkg -i` / GUI launch yet performed on a Linux desktop).
...
- All platform installers run and launch — **PARTIALLY MET**: the Windows installation `.exe` is now built, installs, runs and uninstalls cleanly (2026-09-19); the GTK application `.deb` **is now built** (2026-09-19, WSL Fedora 44, see 9.12) but has not been installed or launched on a Linux desktop; and the macOS `.app` was still never launched in a GUI session. See `docs/INSTALL.md` and `docs/changelog/0.1.0.md` §4.
...
| G9 Docs+Packaging | 15 | 150 | ✅ 15/15 rows present; 9.11 build+sign+binary-exec verified, **GUI launch NOT performed**; 9.13 Windows installer **compiled + install/uninstall-verified 2026-09-19**; **9.12 GTK .deb BUILT 2026-09-19 in WSL** (hicolor icons + Icon=@APPID@) |

...
| 10.2 | Build final artifacts for all platforms | 10m | G9.11-9.14 | macOS .app, Linux .deb, Windows `.exe`, WASM. **All six release assets present and checksum-verified 2026-09-24** (`shasum -a 256 -c dist-release/CHECKSUMS.txt` → 9/9 OK from repo root; note the GTK `.deb` copy on macOS is `dist-release/khostty_0.1.0_amd64.deb`, the `dist/` path is the WSL build location). **Windows installer built 2026-09-19**: `Khostty-0.1.0-windows-x86_64-setup.exe`, 19,766,307 B, `sha256 4070e89e…095546`, ver `0.1.0.0`, compiled with Inno Setup 6.7.1 on `kooshapari-desk` and install/uninstall-verified (see 9.13). **GTK application `.deb` BUILT + INSTALL-VERIFIED 2026-09-19 in WSL Fedora 44 on `kooshapari-desk`**: `khostty_0.1.0_amd64.deb`, 18,143,964 B, `sha256 63d4e615…de0` (see 9.12). |
...
| 10.6 | Create GitHub release with artifacts | 10m | 10.3-10.4 | **PARTIAL — tag pushed; release creation pending operator approval.** `v0.1.0` pushed (tag object `7e1684f6` → `202aad543`). The 6-asset `gh release create` (macOS zip, WASM tarball, library `.deb`, GTK `.deb`, Windows setup.exe, `CHECKSUMS.txt`; notes `docs/changelog/0.1.0.md`) was **deferred to the phinbox approval gate**; no release exists yet, and the gate must not be self-approved. Original request `hook-a683332d46ba9fff95e5a91b0f239118` **expired unapproved**; re-issued 2026-09-24 as **`hook-ea20989dca45c9e2c6c1c915ba817571`** (inbox page opened for the operator) after fixing two runbook defects: the GTK `.deb` asset path (WSL `dist/` → macOS `dist-release/`) and bare `gh` resolving the `upstream` remote to `ghostty-org/ghostty` and refusing (now pinned `--repo KooshaPari/Khostty`, ADMIN verified). All 6 assets confirmed present on disk 2026-09-24. Exact command: RELEASE.md §7.1. |
...
| G10 Release | 9 | 90 | 🔶 IN PROGRESS — 7/9 DONE (10.1–10.4, 10.7, 10.8 incl. all artifacts built and verified: Windows installer, GTK app `.deb` incl. headless GUI launch, macOS `.app` GUI keystroke round-trip, tag pushed; announce docs reconciled 2026-09-24); 10.5 PARTIAL (crates.io + npm live; PyPI blocked on credential); 10.6 PARTIAL (release re-requested 2026-09-24, pending phinbox approval); 10.9 SCHEDULED 2026-10-01 10:00 PDT |
| **TOTAL** | **115** | **~1150m (19.2h)** | **112 DONE / 2 OPEN (10.5 PyPI credential, 10.6 release approval) + 10.9 SCHEDULED (2026-10-01 10:00 PDT)** |
...
> **Refresh 2026-09-24:** pending set is now exactly two — G10.5 (PyPI credential) and
> G10.6 (release approval); G10.2/G10.8 closed, G10.9 scheduled for 2026-10-01.

...
| G10 | Release Artifacts | 9 | 90m | IN PROGRESS (5 of 9 DONE: 10.1–10.4, 10.7; 10.5 PARTIAL — crates.io published 2026-09-20, PyPI+npm blocked on credentials; 10.6 PARTIAL — tag pushed, GitHub release pending phinbox approval; 10.8 docs updated 2026-09-24; 10.9 scheduled post-close) | MEDIUM |
...
published 2026-09-20 (API-verified); PyPI blocked on a missing credential; npm
blocked on a revoked token. **10.6 is PARTIAL**: tag pushed; `gh release create`
...
| 10.5 | Publish FFI crates/packages (crates.io, PyPI, Go, npm) | 10m | G5/G6/G7 | **PARTIAL — crates.io PUBLISHED 2026-09-20; PyPI + npm BLOCKED on credentials.** `khostty-vt` 0.1.0 is live on crates.io (API `newest_version=0.1.0`, downloaded-crate checksum matches, `published_by KooshaPari`; 199/199 tests passed first). PyPI: **no credential exists on the machine** (no `.pypirc`, keyring, env var, netrc, or OIDC); wheel + sdist prebuilt in `khostty-python/dist/`; `uv publish` runs once an operator token is provided; name `khostty-vt` free. npm: stored token **revoked** (401 on `whoami`); needs fresh `/opt/homebrew/bin/npm login --auth-type=web` browser approval; staged package verified (27 files, 324.6 kB, shasum `2754b0a4`); name `khostty-libghostty-vt-wasm` free. Go — source module only, no registry publish needed. **Consumer caveat (documented, not vendored):** the Rust crate looks for a prebuilt native library at `GHOSTTY_VT_LIB_DIR` or `../zig-out/lib`, `../build/lib`, `../dist/lib`; absent that it typechecks but **does not link** — the crate README spells this out; that is the accepted resolution of open decision 1 (RELEASE.md §7). |
...
- FFI packages published or explicitly deferred (with reason) — **PARTIAL**: crates.io **published** 2026-09-20; PyPI (no credential exists) and npm (revoked token) explicitly blocked with recorded reasons (WBS 10.5, `docs/RELEASE.md` §7).
...
| G10 Release | 9 | 90 | 🔶 IN PROGRESS — 5/9 DONE (10.1–10.4, 10.7 incl. all artifacts built and verified: Windows installer, GTK app `.deb` incl. headless GUI launch, macOS `.app` GUI keystroke round-trip, tag pushed); 10.5 PARTIAL (crates.io live; PyPI+npm blocked on credentials); 10.6 PARTIAL (release pending phinbox approval); 10.8 docs updated 2026-09-24; 10.9 scheduled |
